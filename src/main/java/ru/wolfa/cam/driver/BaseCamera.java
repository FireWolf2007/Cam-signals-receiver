...
public abstract class BaseCamera implements Camera {
	...
	/* (non-Javadoc)
     * @see ru.wolfa.cam.driver.Camera#callPosition(java.util.function.Consumer, int)
     */
	@Override
	...
	/* (non-Javadoc)
     * @see ru.wolfa.cam.driver.Camera#executeCmd(java.util.function.Consumer, java.lang.String, int)
     */
	@Override
    abstract public void executeCmd(Consumer<ByteArrayResource> callback, String direction, int timeInMillis);

	/* (non-Javadoc)
     * @see ru.wolfa.cam.driver.Camera#executeCmd(java.util.function.Consumer, java.lang.String, int, java.lang.String, java.lang.String, java.lang.String)
     */
	@Override
    public void executeCmd(Consumer<ByteArrayResource> callback, String direction, int timeInMillis, String CMD_PARAM, String CMD_STOP, String queryPath) {
		getClient().get().uri(builder -> builder.queryParam(CMD_PARAM, direction).path(queryPath).build()).retrieve()
				.bodyToMono(ByteArrayResource.class).subscribe(c -> {
					...
					getClient().get().uri(builder -> builder.queryParam(CMD_PARAM, CMD_STOP).path(queryPath).build()).retrieve()
							.bodyToMono(ByteArrayResource.class).subscribe(d -> {
								log.trace("STOP cam");
								callback.accept(d);
							});
				});
